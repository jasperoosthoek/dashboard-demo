...
const DashboardPage = () => {
  ...
  return (
    <Container className="container-list">
      ...
      <Row className="mb-4">
        <Col md={3}>
          <Card body>{text`active_projects`} <strong>{runningProjects?.length}</strong></Card>
        </Col>
        <Col md={3}>
          <Card body>{text`outstanding_invoices`} <strong>{formatCurrency(openInvoices?.reduce((sum, inv) => sum + inv.amount, 0) || 0)}</strong></Card>
        </Col>
        <Col md={3}>
          <Card body>{text`overdue_tasks`} <strong>{overdueTasks?.length}</strong></Card>
        </Col>
        <Col md={3}>
          <Card body>{text`active_employees`} <strong>{activeEmployees?.length}</strong></Card>
        </Col>
      </Row>
